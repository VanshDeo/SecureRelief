...
const TransparencyPortal = () => {
    ...
    const loadTransparencyData = React.useCallback(async () => {
        // Simulate loading
    ...
    const ViewButton = ({ view, active, onClick }: { view: { id: string, name: string, icon: any }, active: boolean, onClick: (id: string) => void }) => {
        const Icon = view.icon;
        return (
            <button
                onClick={() => onClick(view.id)}
                className={`flex items-center px-4 py-2 rounded-lg font-medium transition-all duration-200 ${active
    ...
    const TransactionRow = ({ transaction }: { transaction: Transaction }) => (
        ...
        >
            ...
            <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${transaction.type === 'donation'
                    ...
                    <div className="mb-10">
                        <Card className="flex flex-col md:flex-row items-center p-6 bg-white shadow-lg border border-red-100">
                            ...
                            <div className="flex-1">
                                ...
                                <Button className="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg shadow" onClick={() => window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' })}>
                                    {ongoingEvent.cta}
                                </Button>
                            </div>
                        </Card>
